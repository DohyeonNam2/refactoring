...
public class StatementPrinter {
    ...
    public String statement() {
        ...
        for (Performance p : invoice.getPerformances()) {
            ...
            switch (play.getType()) {
                case "tragedy":
                    thisAmount = BASE_TRAGEDY_AMOUNT;
                    if (p.getAudience() > Constants.TRAGEDY_AUDIENCE_THRESHOLD) {
                        final int temp = p.getAudience() - Constants.TRAGEDY_AUDIENCE_THRESHOLD;
                        thisAmount += TRAGEDY_EXTRA_PER_AUDIENCE * temp;
                    }
                    break;
                case "comedy":
            ...
            result.append(String.format(
                    "  %s: %s (%s seats)%n",
                    play.getName(),
                    frmt.format(thisAmount / CENTS_PER_DOLLAR),
                    p.getAudience()));
